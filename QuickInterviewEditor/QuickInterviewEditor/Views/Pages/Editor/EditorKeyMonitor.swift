...
struct EditorKeyMonitor: NSViewRepresentable {
  ...
  final class Coordinator {
    ...
    private func handle(_ key: EditorKey, isARepeat: Bool) -> Bool {
      ...
      // Z is a toggle; swallow key auto-repeat so holding it doesn't flicker fit↔restore (and
      // doesn't fall through to a beep). The arrow zoom keys intentionally keep repeating.
      if key == .zoomFit, isARepeat { return true }
