...
private struct SliceCard: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 6) {
      ...
      // The range + snippet block is the reveal target — a click here jumps the transcript and
      // waveform to the clip. Kept off the rename field and the action buttons so it never
      // steals their taps.
      VStack(alignment: .leading, spacing: 6) {
      ...
      .onTapGesture { model.sliceRevealTapped(row.id) }
      HStack(spacing: 8) {
