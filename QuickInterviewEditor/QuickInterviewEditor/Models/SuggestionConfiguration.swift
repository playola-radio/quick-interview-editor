...
extension String {
  private static let configurationWhitespace = CharacterSet(
    charactersIn:
  ...
  fileprivate var isBlank: Bool {
    trimmingCharacters(in: Self.configurationWhitespace).isEmpty
  }

  fileprivate var normalizedConfigurationName: String {
    components(separatedBy: Self.configurationWhitespace)
      .filter { !$0.isEmpty }
      .joined(separator: " ")
      .folding(
