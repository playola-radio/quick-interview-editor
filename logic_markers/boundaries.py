...
def snap_boundaries(
    ...
    *,
    ...
    leading = [s for s in silences if s.start < cs]
    start_status = "padded"
    start_sample = max(0, cs - pad)
    if leading:
        s = max(leading, key=lambda r: r.end)  # latest-ending -> closest to word
        if cs - s.end <= radius:
            start_sample = min(max(s.end - roll, s.start), cs)
            start_status = "snapped"

    # ---- end: nearest silence that extends past the last word ----
    trailing = [s for s in silences if s.end > ce]
    end_status = "padded"
    end_sample = min(total_samples, ce + pad)
    if trailing:
        s = min(trailing, key=lambda r: r.start)  # earliest-starting -> closest
        if s.start - ce <= radius:
            end_sample = max(min(s.start + roll, s.end), ce)
            end_status = "snapped"

    start_sample = max(0, min(start_sample, cs))
    end_sample = min(total_samples, max(end_sample, ce))
    return Boundaries(
