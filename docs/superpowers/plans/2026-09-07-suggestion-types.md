...
The [broader Intro follow-up](2026-09-08-broader-intros.md) also supersedes handoff-only Intro discovery for fresh searches.
...
**Files:** Create `Models/SuggestionRunWire.swift` and a focused internal live-process adapter if needed; modify `Core/CutSuggestClient.swift`, `Core/LiveCutSuggester.swift`, `Models/CutSuggestOptions.swift`, `Models/CutSuggestionWire.swift`, `Models/SuggestionRun.swift`; extend `QuickInterviewEditor/QuickInterviewEditorTests/Core/CutSuggestClientTests.swift`, `LiveCutSuggesterTests.swift`, `Models/CutSuggestionWireTests.swift`.

- [ ] Add `snapshot: SuggestionRunSnapshot?`, `mode: SuggestionSearchMode`, and `journalDirectory: URL?` to `CutSuggestRequest` with initializer defaults preserving old fixture call sites. New app runs always set a snapshot; nil is only legacy compatibility. Define `SuggestionSearchMode: String, Codable, Sendable` with `fresh`, `automatic`, `resume`. Pin option versions and `stage1Window`/`stage1Step` in the snapshot (backward-decoding defaults 130/110); encode resumed requests from the immutable values and verify them against the durable original request. Don't silently change the current model selection as part of this feature.
...
- [ ] Remove the valid-empty rejection in **both** `LiveCutSuggester.completedEvent` and the page model's completion path (the latter is fully replaced in Task 13). A well-formed completed empty list is success; malformed/all-invalid provider data must be distinguished upstream and remain an error. Preserve actual diagnostic metadata through `.diagnostic(String)` and a model-owned last-run diagnostic retained after empty completion (clear on a new launch); do not substitute arbitrary activity progress. Zero length alone is not failure. Preserve an optional backward-compatible checkpoint `failureMessage` for actionable retry/input-size explanations across reopen.
- [ ] Add deterministic live-adapter tests. The existing adapter directly constructs `SpawnedProcess`; introduce a small internal injectable runner/process handle that delegates live operations to that unchanged process-group implementation, so controlled test streams can exercise cancellation without a real provider helper. Cover: checkpoint line recognition; stale revision ignored; cancellation terminates the child and doesn't emit completion; stderr without progress doesn't misclassify auth; credential stays out of encoded request, checkpoint, and argv. Avoid launching the actual provider helper in unit tests.
...
Implementation refinement from recovery integration review: add `SuggestionRecoveryCapture` containing a checkpoint and its encoded portable archive, with narrow `capture(owner, runID, minimumPythonRevision)`, `restore(owner, archive)`, and `resolveOwner(persistedID, documentURL, sourceFingerprint, transcriptHash)` client operations. Use one authoritative capture/validation implementation. Capture the Python checkpoint before its referenced records; include already completed records ahead of that checkpoint. Preserve Python-owned JSON bytes/integrity, validate semantic immutable request identity and raw response hashes, and merge completed records independently of the two revisioned components. Before an offline ready result can be applied, verify aggregate integrity too: use the original canonical Python bytes (for example, remove the top-level integrity member without re-encoding the remaining JSON), rather than cloning Python float/Unicode serialization with JSONEncoder. Check a real Python-produced fixture and a valid-JSON corruption case; nested fields named integrity must not confuse the top-level check. Initial prepare supports a revision-zero, manifest-only capture before Python starts.

Publish the checkpoint, owner, and matching archive together through an atomic document sink operation; ordinary edits preserve that archive. Synchronous background document snapshots only copy this coherent immutable content and never wait on an actor/main thread. Save captures the latest accepted coherent checkpoint/archive pair; newer local paid work remains in the durable journal until captured. Applied runs retain their archive until disk confirmation. Tests must prove a suspended newer capture cannot save a mismatched checkpoint/archive pair.

Resolve owner/location before exposing search actions. The existing same-instance Save As URL observer runs after AppKit's first write: synchronously gate run mutations at the observed transition, cancel the old attempt, durably clone recovery, then publish the fresh owner and archive together and dirty the destination for its corrected save. On reopen, perform the same owner collision resolution before any recovery mutation, including if a crash left the initial copied owner in the destination. Do not infer duplication from `existingFile == nil`; first save and a genuine move retain ownership. The current document API cannot promise a destination-aware owner change inside the first Save As write, but both live and reopened copies must isolate journals before mutating them.

Implementation refinement from Task 12 quality review: applying a run must not prevent another search in an untitled or not-yet-saved project. Retain per-run Swift manifests in an owner-owned `manifests/` directory alongside the current owner manifest. Preparing B over A is authorized only when `preparation.lastAppliedRunID` explicitly identifies A and A validates as ready; an unfinished run is never silently replaced. Persist A's retained-applied manifest before publishing B and preserve A's Python files. Track ordered applied ancestors. Run-specific capture/discard/confirmation can address retained A without deleting current B. Discarding B falls back to retained A; on reopen the document's loaded lastAppliedRunID determines whether A is offered for explicit recovery or recognized as already applied. Thus a crash before either save preserves both B's unfinished work and A's completed work.

The portable archive includes a fixed, flat list of retained single-run payloads, validated with the same strict rules and original Python bytes; reject duplicate run IDs and nested retained payload chains. Disk confirmation of saved A retires only A and its applied ancestors, preserving B. Disk confirmation of successfully applied successor B may retire its retained applied ancestry: that saved successor proves the explicitly replaced state and current clips are durable. Keep current metadata/archive coherent when retained entries are cleaned. Test untitled and saved A→B, B failure/discard, saved-successor cleanup, and restoration with no local files.

Owner resolution receives an ephemeral document instance ID and performs location/copy/claim isolation before comparing source/transcript and returning a stale-discard diagnostic. A changed-transcript second window must never receive a destructive capability for the first window's owner. Test two nil-URL windows where discarding the second window's stale recovery leaves the first journal intact.
...
- [ ] When reopening matching source/transcript without a restored document owner, use `recoverableOrphans` to offer an explicit recovery choice. Never silently select a hash match, including a single match. Selection resolves/claims isolated ownership and publishes a validated capture before offering Resume; Cancel leaves the document unchanged. Cover multiple matches and a still-open original owner.
...
**Files:** Create `Views/Pages/CutSuggestions/SuggestionReviewModel.swift`, `SuggestionReviewView.swift`, `QuickInterviewEditor/QuickInterviewEditorTests/Views/Pages/CutSuggestions/SuggestionReviewTests.swift`; modify `CutSuggestionsPageModel.swift`, `CutSuggestionsPageView.swift`, `Models/CutSuggestionPresentation.swift`, `Models/SuggestionNumbering.swift`, `Views/Pages/Editor/EditorModel.swift`; extend `CutSuggestionsPageTests.swift`, `Views/Pages/Editor/EditorClipBandsTests.swift`.
...
**Files:** Create focused `Core/ExportCopyClient.swift` if needed for injectable filesystem operations; modify `Models/ExportNaming.swift`, `Views/Pages/Editor/EditorModel.swift`, `EditorView.swift`; create `Views/Pages/Editor/ExportReviewModel.swift`, `ExportReviewView.swift`, `QuickInterviewEditor/QuickInterviewEditorTests/Views/Pages/Editor/ExportReviewTests.swift`; extend `Models/ExportNamingTests.swift` and `Views/Pages/Editor/EditorExportRemovalTests.swift`.
...
- [ ] Write an injected-filesystem test where another writer creates `ID 1.aiff` after preflight: no overwrite, no silent `ID 1 2.aiff`, one new review mapping. Add cancellation-after-partial-copy, confirmation then second race, all-safe no-dialog, and old export tests. The current copy helper uses FileManager directly; add a focused Sendable export copy/list dependency for the race tests, retaining real temporary-directory coverage. Keep file protection there rather than introducing a general filesystem framework; no live Logic automation is needed.
...
**Files:** Create focused `cut_suggester/intro_refinement.py` and `tests/test_intro_refinement.py`; modify `cut_suggester/cutter.py`, `configured_run.py`, `configured_discovery.py`, and fresh configured Swift discovery-version defaults; extend `tests/test_configured_run.py`, create `tests/test_suggestion_editorial_eval.py`, `evals/cut_suggestions/editorial_runner.py`; extend `QuickInterviewEditor/QuickInterviewEditorTests/Views/Pages/Editor/EditorSuggestionFlowTests.swift`; update `README.md`, `evals/cut_suggestions/README.md`; create `docs/superpowers/reviews/2026-09-07-suggestion-types-validation.md` during execution.

- [ ] Address actual live synthetic failures recorded in `.context/editorial-live-findings.md`: the provider merged repeated IDs4–5/6–7 into4–7 and selected Intro23–28 instead of24–25. Preserve the shared tuned partition/classification prompt bytes. Add an opt-in configured Intro refinement after raw classification and before candidate construction/duration/deduplication, showing globally numbered sentences and configured Intro guidance. Return an explicit result for every deterministic proposal reference; allow zero/one/multiple contained complete takes, with strict reference uniqueness/membership, integer in-bounds ranges, nonblank labels, and no overlapping takes within one proposal. Bound prompt batches. Pass Spotlight records through unchanged. Use the existing validated-request journal seam so a saved refinement survives interruption without another paid request. Assign stable candidate UUIDs and extract fields only after final spans; changed spans never inherit old candidate fields.
- [ ] Strengthen only the generic imaging take instructions: consecutive identical independently complete performances each get a separate clip; do not combine them because text, speaker, or label matches. Preserve nested-ID, incidental-mention, and cross-type overlap rules. Introduce an explicit new configured discovery behavior version for fresh runs, gating both refinement and revised generic prompt; captured older versions retain their old path. Preserve legacy helper/eval defaults. Test version dispatch, disabled-Intro no extra request, recorded coarse spans, split repeated Intros/IDs, rejected fragments, malformed refinement responses, final field IDs, and interrupted refinement replay.
